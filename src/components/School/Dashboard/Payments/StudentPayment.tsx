...
const SchoolStudentPayment = () => {
  ...
  const payWithPaystack = () => {
    ...
    if (studentCount > 1000) {
      toast.error("Maximum 1000 students allowed. Please contact support for larger orders.");
      return;
    }
      ...
      <div className="flex justify-between lg:p-2 flex-col md:flex-row gap-4">
        <div className="flex flex-col gap-4 bg-white rounded-md p-2 py-4 w-full md:w-[48%]">
          ...
          <div className="mt-4 border-t border-gray-200 pt-4">
            <label htmlFor="couponCode" className="text-sm font-semibold block mb-2">
              Have a Coupon Code?
            </label>
          ...
          {couponApplied && discountAmount > 0 && (
            <div className="mt-3 p-2 bg-green-50 border border-green-200 rounded">
              <p className="text-xs text-green-700 font-semibold text-center">
                You're saving ₦{formatCurrency(discountAmount + (discountAmount * VAT_RATE))}!
              </p>
            </div>
          )}
        </div>
